...
class Home extends CI_Controller
{
    ...
    // )
    // );
    // $CI->dbforge->add_column('listing', $col);
    ...
    {   $home_page = get_frontend_settings('home_page');
        if($home_page == 1){
    ...
    function make_geo_json_for_map($listings = array(), $param2 = "", $listing_id = "")
    {
        ...
        foreach ($listings as $key => $listing) {
            ...
            $listing_tags = isset($listing['tags'])?explode(',', $listing['tags']):array();
    ...
    function listing_form($action = "")
    { // This function only shows the form for adding and editing the listing.
        if ($action == "")
            redirect(site_url('home/listings'), 'refresh');
    ...
    function send_contact_message()
    {
        ...
        if ($this->crud_model->check_rechaptcha() || get_settings('recaptcha_status') == 0) {
    ...
    {
        $page_data['user_details'] = $this->user_model->get_all_users($user_id)->row_array();
